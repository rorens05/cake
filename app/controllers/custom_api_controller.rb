class CustomApiController < ApplicationController
  ...
  def create_cod_order
    order = Order.new(ordered_at: Time.now, customer_id: params[:customer_id], size_id: params[:size_id], no_of_items: params[:items], payment_method: params[:payment_method], delivery_location: params['delivery_location'], date_to_be_delivered: params['delivery_date'] )
