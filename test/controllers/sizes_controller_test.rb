...
class SizesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create size" do
    assert_difference('Size.count') do
      post sizes_url, params: { size: { label: @size.label, price: @size.price, product_id: @size.product_id } }
    end
  ...
  test "should update size" do
    patch size_url(@size), params: { size: { label: @size.label, price: @size.price, product_id: @size.product_id } }
